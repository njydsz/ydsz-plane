...
var (
	...
	ErrValidation = New(string(CodeValidation), "参数校验失败", http.StatusUnprocessableEntity)

	// ErrNotImplemented 功能尚未实现（HTTP 501 Not Implemented）。
	//
	// 触发场景：模块处于规划/占位阶段（如 AI 能力未接入时返回占位错误），
	// 前端据此展示 "功能开发中" 状态而非笼统的 500。
	ErrNotImplemented = New("NOT.IMPLEMENTED", "功能尚未实现", http.StatusNotImplemented)
	...
	ErrUnauthorized = New(string(CodeAuth), "未认证或凭证已失效", http.StatusUnauthorized)
	...
	ErrForbidden = New(string(CodeForbidden), "没有执行该操作的权限", http.StatusForbidden)
	...
	ErrNotFound = New(string(CodeNotFound), "资源不存在", http.StatusNotFound)
	...
	ErrRateLimited = New(string(CodeRateLimit), "请求过于频繁，请稍后再试", http.StatusTooManyRequests)
	...
	ErrInternal = New(string(CodeInternal), "服务内部错误", http.StatusInternalServerError)
	...
	ErrTokenExpired = New(string(CodeAuthTokenExpired), "登录已过期，请重新登录", http.StatusUnauthorized)
	...
	ErrVersionConflict = New(string(CodeIssueVersion), "数据已被他人修改，请刷新后重试", http.StatusConflict)
	...
	ErrInvalidTransition = New(string(CodeIssueInvalidState), "当前状态不允许该流转", http.StatusUnprocessableEntity)
	...
	ErrWBSDepthExceeded = New(string(CodeIssueDepthLimit), "工作项层级最多支持三级", http.StatusUnprocessableEntity)
